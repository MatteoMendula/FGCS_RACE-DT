...
def get_Hps_folder(asset_name):
    HPS_FOLDER = f"./RC_selected_hps/{asset_name}/cae_pytorch_rc_mlp_aggregated_prediction_val_split_withMAC_merged/"
    return HPS_FOLDER

def getResultsFolder(asset_name):
    RESULTS_FOLDER = f"./time_only/NASA_results_val_selected_hps_{asset_name}/cae_pytorch_rc_mlp_aggregated_prediction_val_split_withMAC/"
...
if __name__ == "__main__":
    ...
    # save torch model on disk
    ecaeAE_model_path = os.path.join(getResultsFolder(ASSET_NAME), f"{n_rc_layers}_rc_layers", f"{ASSET_NAME}_cae_pytorch_rc_pred_results_val_split.pt") 
    # create directory if it doesn't exist
    os.makedirs(os.path.dirname(ecaeAE_model_path), exist_ok=True)
    torch.save(cae_model_pytorch.state_dict(), ecaeAE_model_path)

    # get ecaeAE_model_path file size in bytes
    file_size = os.path.getsize(ecaeAE_model_path)
    results["cae_model_file_size_bytes"] = file_size
    ...
    base_hp_folder = get_Hps_folder(ASSET_NAME)
    hp_file_path = os.path.join(base_hp_folder, f"{n_rc_layers}_rc_layers", f"{ASSET_NAME}_cae_pytorch_rc_pred_results_val_split.csv")
    if not os.path.exists(hp_file_path):
        raise FileNotFoundError(f"Hyperparameter file not found: {hp_file_path}. Ensure the path is correct.")
    
    df = pd.read_csv(hp_file_path)
    best_hps_params_string = df["best_hps_params"][0]
    # parse the string to a list of dictionaries
    best_params_list = eval(best_hps_params_string)
    ...
    # save final MLP model on disk
    final_mlp_model_path = os.path.join(getResultsFolder(ASSET_NAME), f"{n_rc_layers}_rc_layers", f"{ASSET_NAME}_cae_pytorch_rc_pred_results_val_split_mlp.pkl")
    with open(final_mlp_model_path, 'wb') as f:
        pickle.dump(final_mlp, f)
    # get final_mlp_model_path file size in bytes
    final_mlp_file_size = os.path.getsize(final_mlp_model_path)
    results["final_mlp_model_file_size_bytes"] = final_mlp_file_size
